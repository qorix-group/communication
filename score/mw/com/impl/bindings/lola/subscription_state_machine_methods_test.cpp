...
//         // Specifically, it's important that the Unsubscribe is recorded so that when
...
//
// TEST_F(StateMachineMethodsNotSubscribedStateFixture,
// CallingSubscribeAfterUnsettingReceiveHandlerWillNotRegisterHandler)
// {
//     StrictMock<MockFunction<void()>> receive_handler{};
//
...
//
//     // Expecting that the receive handler registered while in NotSubscribed state will never be called and the
//     handler
...
//
// TEST_F(StateMachineMethodsSubscriptionPendingStateFixture,
// CallingGetTransactionLogIndexReturnsValidTransactionLogIndex)
// {
//     // Given that the state machine is currently in subscription pending
//     EnterSubscriptionPending(max_num_slots_);
//
