...
///
/// RegisterPid is used by ApplicationIdPidMapping, which resides in shared memory so we cannot rely on dynamic
/// dispatch. We would normally create an interface for a RegisterPidMock such that ApplicationIdPidMapping depends only
/// on the interface and the test itself can add the dependency on the mock (which depends on gtest). Since we cannot do
/// this, we want to avoid that ApplicationIdPidMapping depends on gtest in production so we instead use this simple
/// fake class.
