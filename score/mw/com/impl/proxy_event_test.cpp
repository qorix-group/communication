...
TYPED_TEST(ProxyEventFixture, ReceiveDataFromProxy)
{
    ...
    EXPECT_TRUE(Base::proxy_event_.Subscribe(7U));
...
TYPED_TEST(ProxyEventFixture, SamplePtrLimitsAreEnforced)
{
    ...
    EXPECT_TRUE(Base::proxy_event_.Subscribe(max_num_samples));
...
TYPED_TEST(ProxyEventDeathTest, DieOnUnsubscribingWhileHoldingSamplePtrs)
{
    ...
    ASSERT_TRUE(Base::proxy_event_.Subscribe(max_num_samples));
