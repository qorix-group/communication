 ...
 ********************************************************************************/
#include "score/mw/com/test/generic_skeleton_method/consumer.h"
...
const InstanceSpecifier kInstanceSpecifier =
    InstanceSpecifier::Create(std::string{"test/generic_skeleton_method/TestMethods"}).value();
...
int run_consumer()
{
    ...
    {
        std::cerr << "Consumer: Could not create ProcessSynchronizer" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    {
        std::cerr << "Consumer: CreateProxy failed" << std::endl;
    ...
    if (!consumer.CallMethodWithInArgsAndReturn(kTestValueA, kTestValueB, MethodConsumer<TestMethodProxy>::CopyMode::WITH_COPY))
    {
        std::cerr << "Consumer: CallMethodWithCopy failed" << std::endl;
        process_synchronizer_result->Notify();
        return EXIT_FAILURE;
    }
    ...
    if (!consumer.CallMethodWithInArgsAndReturn(kTestValueA, kTestValueB, MethodConsumer<TestMethodProxy>::CopyMode::ZERO_COPY))
    {
        std::cerr << "Consumer: CallZeroCopyMethod failed" << std::endl;
        process_synchronizer_result->Notify();
        return EXIT_FAILURE;
    }
